...
func (r *SyncReconciler) Reconcile(ctx context.Context, req reconcile.Request) (res reconcile.Result, retErr error) {
	...
	return reconcile.Result{}, r.handleClusterAddOrUpdate(ctx, cluster.DeepCopy(), buildClusterConfigInSyncer, nil)
}
...
func (r *SyncReconciler) handleClusterAddOrUpdate(ctx context.Context, cluster *clusterv1beta1.Cluster, buildClusterConfigFunc buildClusterConfigFunc, filterForSdkFunc filterForSdkFunc) error {
	logger := ctrl.LoggerFrom(ctx)

	allResources, validResources, pendingWildcards, err := r.getResources(ctx, cluster, filterForSdkFunc)
	if err != nil {
	...
	if hasWildcards {
		...
		wildcardResources, err := r.processWildcardResources(ctx, pendingWildcards, singleMgr, cluster.Name, filterForSdkFunc)
		if err != nil {
		...
		validResources = append(validResources, wildcardResources...)
		logger.Info("Added resources from wildcards to the sync list for cluster", "resources", strconv.Itoa(len(wildcardResources)), "cluster", cluster.Name)
	}

	// double check for cluster without resources
...
func (r *SyncReconciler) getResources(ctx context.Context, cluster *clusterv1beta1.Cluster, filterForSdkFunc filterForSdkFunc) ([]*searchv1beta1.ResourceSyncRule, []*searchv1beta1.ResourceSyncRule, []*searchv1beta1.ResourceSyncRule, error) {
	...
	var validResources []*searchv1beta1.ResourceSyncRule
	...
	for _, registry := range registries {
		...
		for gvr, rule := range resources {
			...
			if filterForSdkFunc == nil || filterForSdkFunc(ctx, gvr, rule) {
	...
	return allResources, validResources, pendingWildcards, nil
}
...
func (r *SyncReconciler) generateAgentYaml(ctx context.Context, cluster *clusterv1beta1.Cluster) error {
	...
	agentYml, err := r.renderYamlFile(cluster, base64.StdEncoding.EncodeToString(certData), base64.StdEncoding.EncodeToString(keyData))
	if err != nil {
...
func (r *SyncReconciler) processWildcardResources(
	...
	clusterName string,
	filterForSdkFunc filterForSdkFunc,
	...
	for _, nr := range wildcards {
		...
		for _, apiResource := range resources.APIResources {
			...
			gvr, err := parseGVR(resourceRule)
			if err != nil {
				return nil, errors.Wrapf(err, "error in parsing GVR")
			}
			if filterForSdkFunc != nil && !filterForSdkFunc(ctx, gvr, resourceRule) {
				continue
			}
