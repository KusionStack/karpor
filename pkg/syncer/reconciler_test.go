...
func TestSyncReconciler_getResources(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			r := &SyncReconciler{client: fake.NewClientBuilder().WithRuntimeObjects(tt.srs...).WithScheme(scheme.Scheme).Build()}
			allResources, _, _, err := r.getResources(context.TODO(), tt.cluster, nil)
			if tt.wantErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestSyncReconciler_processWildcardResources(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r := &SyncReconciler{}
			got, err := r.processWildcardResources(context.TODO(), tt.wildcards, &fakeSingleClusterSyncManager{mockManager}, tt.clusterName, nil)
...
func TestSyncReconciler_handleClusterAddOrUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := r.handleClusterAddOrUpdate(context.TODO(), tt.cluster, buildClusterConfigInSyncer, nil)
			if tt.wantErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
