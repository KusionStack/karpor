	...
	if childRelation.Type == "OwnerReference" {
		...
		if errors.IsNotFound(err) {
			...
			if childRelation.Type == "JSONPath" {
				resourceGraph, err = GetChildrenByJSONPath(childrenResList, ctx, client, obj, childRelation, gvk, objResourceNode, relationshipGraph, resourceGraph)
				if err != nil {
					return nil, err
				}
			} else if childRelation.Type == "Selector" {
				resourceGraph, err = GetChildrenByLabelSelector(childrenResList, ctx, client, obj, childRelation, gvk, objResourceNode, relationshipGraph, resourceGraph)
				if err != nil {
					return nil, err
				}
			} else {
...
// GetChildrenByJSONPath returns a graph that includes all of the child resources for the current obj that are described by the JSONPath type childRelation
func GetChildrenByJSONPath(childrenResList *unstructured.UnstructuredList, ctx context.Context, client *dynamic.DynamicClient, obj unstructured.Unstructured, childRelation *Relationship, childGVK schema.GroupVersionKind, objResourceNode ResourceGraphNode, relationshipGraph graph.Graph[string, RelationshipGraphNode], resourceGraph graph.Graph[string, ResourceGraphNode]) (graph.Graph[string, ResourceGraphNode], error) {
	klog.Infof("Using direct references to find children...\n")
	var jpMatch bool
	var err error
	for _, childRes := range childrenResList.Items {
		if childRelation.AutoGenerated {
			jpMatch, err = JSONPathMatch(childRes, obj, childRelation.JSONPath)
		} else {
			jpMatch, err = JSONPathMatch(obj, childRes, childRelation.JSONPath)
		}
		if jpMatch && err == nil {
			klog.Infof("Child resource found for kind %s, name %s based on JSONPath.\n", obj.GetKind(), obj.GetName())
			...
			childGVKOnGraph, _ := FindNodeOnGraph(relationshipGraph, childGVK.Group, childGVK.Version, childGVK.Kind)
...
// GetChildrenByLabelSelector returns a graph that includes all of the child resources for the current obj that are described by the LabelSelector type childRelation
func GetChildrenByLabelSelector(childrenResList *unstructured.UnstructuredList, ctx context.Context, client *dynamic.DynamicClient, obj unstructured.Unstructured, childRelation *Relationship, childGVK schema.GroupVersionKind, objResourceNode ResourceGraphNode, relationshipGraph graph.Graph[string, RelationshipGraphNode], resourceGraph graph.Graph[string, ResourceGraphNode]) (graph.Graph[string, ResourceGraphNode], error) {
	klog.Infof("Using label selectors to find children...\n")
	for _, childRes := range childrenResList.Items {
		if labelsMatch, err := LabelSelectorsMatch(obj, childRes, childRelation.SelectorPath); labelsMatch && err == nil {
			klog.Infof("Child resource found for kind %s, name %s based on %s.\n", obj.GetKind(), obj.GetName(), childRelation.SelectorPath)
			klog.Infof("Child resource is: kind %s, name %s.\n", childRes.GetKind(), childRes.GetName())
			klog.Infof("---------------------------------------------------------------------------\n")
			cgv, _ := schema.ParseGroupVersion(childRes.GetAPIVersion())
			childResourceNode := ResourceGraphNode{
