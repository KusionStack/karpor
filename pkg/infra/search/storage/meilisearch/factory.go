...
func (s *SearchStorageGetter) GetSearchStorage() (storage.SearchStorage, error) {
	ms, err := NewStorage(s.cfg.Addresses, s.cfg.Password)
	if err != nil {
		return nil, err
	}
	return ms, nil
...
type Config struct {
	Addresses []string `env:"MS_ADDRESSES"`
	UserName  string   `env:"MS_USER"`
	Password  string   `env:"MS_PASSWORD"`
}
