...
import { DeploymentUnitOutlined, SearchOutlined, BulbOutlined, VerifiedOutlined, SortAscendingOutlined, SortDescendingOutlined, ArrowDownOutlined, ArrowUpOutlined } from "@ant-design/icons";
...
const Cluster = () => {
  ...
  useEffect(() => {
    ...
    if (!searchValue) {
      ...
      if (newValue?.length === 1) {
        ...
        res?.forEach((item: any) => {
          if (newValue?.every((innerValue: string) => item?.metadata?.name?.toLowerCase()?.includes(innerValue))) {
            newShowPageData.push(item)
          };
        });
      }
  ...
  function goCertificate(item) {
    navigate(`/cluster/certificate?cluster=${item?.metadata?.name}&apiVersion=${item?.apiVersion}`)
  ...
  function goDetailPage(item) {
    navigate(`/insightDetail/cluster?from=cluster&type=cluster&cluster=${item?.metadata?.name}&apiVersion=${item?.apiVersion}`);
  }

  const tabsList = [
    {
      label: <div style={tabStyle}><DeploymentUnitOutlined style={iconStyle} />全部集群<span style={numberStyle}>{summary?.totalCount}</span></div>, value: "all"
    },
    { label: <div style={tabStyle}><BulbOutlined style={{ ...iconStyle, background: 'rgba(255,77,79, 0.08)', color: '#FF4D4F' }} />异常集群<span style={numberStyle}>{summary?.unhealthyCount}</span></div>, value: "exception" },
    { label: <div style={tabStyle}><VerifiedOutlined style={{ ...iconStyle, background: 'rgba(82,196,26, 0.08)', color: '#52C41A' }} />健康集群<span style={numberStyle}>{summary?.healthyCount}</span></div>, value: "healthy" },
  ...
  return (
    <div className={styles.container}>
      <div className={styles.actionBar}>
        ...
        {
          (pageData && pageData?.length > 0) && <Button type="primary" onClick={join}>
            接入集群
          </Button>
        }
      </div>
      {
        (!pageData || !pageData?.length) ?
            ...
            <div className={styles.emptyData}>
              <div className={styles.left}>
                ...
                <div className={styles.btnBox}>
                  <Button type="primary" onClick={join}>接入集群</Button>
                </div>
              </div>
          ...
          : <div className={styles.content}>
            ...
            <div className={`${styles.pageContent} ${styles[`pageContent_${triangleLeftOffestIndex}`]}`}>
              <div className={styles.toolBar}>
                ...
                <Button type="link" style={{ color: '#646566' }} onClick={() => handleSort('name')}>名称排序
                  {
                    sortParams?.orderBy === 'name' && (
                      sortParams?.isAsc ? <SortDescendingOutlined style={orderIconStyle} /> : <SortAscendingOutlined style={orderIconStyle} />
                    )
                  }
                </Button>
                <Button type="link" style={{ color: '#646566' }} onClick={() => handleSort('timestamp')}>时间排序
                  {
                    sortParams?.orderBy === 'timestamp' && (
                      sortParams?.isAsc ? <ArrowUpOutlined style={orderIconStyle} /> : <ArrowDownOutlined style={orderIconStyle} />
                    )
                  }
                </Button>
              ...
              {
                loading
                  ? <div style={{ height: 300, display: 'flex', justifyContent: 'center', alignItems: 'center' }}><Loading /></div>
                  :
                        ...
                        return (
                          ...
                          >
                            ...
                            <span><Tag style={{ fontSize: 12 }}>定义的标签</Tag></span> */}
                                ...
                                <div className={styles.bottom}>描述：{item?.spec?.description || '--'}</div>
                                <div className={styles.bottom}>{item?.metadata?.creationTimestamp ? utcDateToLocalDate(item?.metadata?.creationTimestamp) : '--'}</div>
                              </div>
                            </div>
                            <div className={styles.right}>
                              {/* TODO: 非owner用户不能操作，所有按钮置灰 */}
                              <Popconfirm placement="topLeft" title={<span style={{ display: 'inline-block', width: 200 }}>删除后，将不会再管控该集群，和更新相关资源。</span>} description="" onConfirm={() => deleteItem(item)}>
                                <Button>删除</Button>
                              </Popconfirm>
                              {/* <Button style={{ margin: '0 16px' }} onClick={() => edit(item)}>编辑</Button> */}
                              <EditPopForm cancel={handleCancel} submit={handleSubmit} btnStyle={{ margin: '0 16px' }} lastDetail={item} setLastDetail={setLastDetail} />
                              <Button onClick={() => goCertificate(item)}>更新证书</Button>
                            </div>
                          </div>
                        );
                      })}
                    </div>
                    : <div style={{ background: '#fff', borderRadius: 8, height: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Empty />
                    </div>
              }
            </div>
