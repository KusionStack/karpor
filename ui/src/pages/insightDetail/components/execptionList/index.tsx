...
const ExecptionList = ({ execptionList, rescan, showDrawer, onItemClick, execptionStat, auditLoading }) => {
      ...
      <div className={styles.header}>
        <div className={styles.header_left}>
          <ExecptionStat
            ...
            statData={{ all: execptionStat?.issuesTotal || 0, high: execptionStat?.severityStatistic?.High || 0, medium: execptionStat?.severityStatistic?.Medium || 0, low: execptionStat?.severityStatistic?.Low || 0 }}
                  ...
                  <>
                    {
                      top5List?.map(item => {
                        ...
                        return <div key={uniqueKey} className={styles.item}
                          onMouseMove={() => setSelectedEventId(uniqueKey)}
                          onMouseOut={() => setSelectedEventId(undefined)}
                          onClick={() => onItemClick(item)}
                        >
                          {
                            selectedEventId === uniqueKey && <div className={styles.itme_tip}>查看事件详情</div>
                          }
                          ...
                          <div className={styles.top}>
                            ...
                            <div className={styles.right}>
                              <div>
                                <span>
                                  发生&nbsp;
                                  <span style={{ fontWeight: 'bold', color: '#646566' }}>{item?.locators?.length}</span>
                                  &nbsp;次
                                </span>
                                &nbsp;采集自
                              </div>
                              <div className={styles.tool}>
                                <ArrowRightOutlined />&nbsp;{item?.issue?.scanner}&nbsp;工具
                              </div>
                            </div>
                          </div>
                    ...
                    <div className={styles.footer} >
                      <span className={styles.btn} onClick={showDrawer}>
                        查看全部异常事件<ArrowRightOutlined />
                      </span>
                    </div>
                  </>
                )
                : <div style={{ height: 372, display: 'flex', justifyContent: 'center', alignItems: 'center' }}><Empty description="无风险异常，再接再厉~"/></div>
            )
        }
      </div>
    </div >
