...
const SourceTable = ({ queryStr, tableName }: IProps) => {
  ...
  const columns = [
    {
      ...
      title: '名称',
      render: (_, record) => {
        return <Button type="link" onClick={() => goResourcePage(record)}>{record?.object?.metadata?.name}</Button>
      }
    },
    {
      dataIndex: 'namespace',
  ...
  async function queryTableData(params) {
    ...
    const response: any = await axios.get(`/rest-api/v1/search?query=${queryStr}&pattern=sql&page=${params?.current || current}&pageSize=${params?.pageSize || pageSize}`);
    if (response?.success) {
  ...
  return (
    <div>
      <div className={styles.table_header}>
        <div className={styles.table_title}>{tableName || '--'}
          {
            urlSearchParams?.type === 'kind' ? null : <span className={styles.tips}>💡 可在上方选择资源后在这里查看对应的详情</span>
          }
        </div>
        <Space style={{ marginBottom: 10 }}>
          <Input disabled placeholder="请输入名称搜索" suffix={<SearchOutlined />} />
        </Space>
      </div>
      <Table columns={columns} dataSource={tableData} rowKey={(record) => {
