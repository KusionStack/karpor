...
const allTagsMock = Array.from({ length: 100 }, (_, i) => `Tag${i + 1}`);

const MutiTag = ({ allTags = allTagsMock }) => {
  ...
  return (
    <div className={styles.appContainer}>
      {allTags.slice(0, showAll ? allTags.length : defaultMaxCount).map((tag: any) => {
        const content = (
      ...
      {allTags.length > defaultMaxCount && !showAll && (
        <div className={styles.toggleButton} onClick={toggleTags}>
          <span>
            更多
            <DoubleLeftOutlined style={{ transform: 'rotate(-90deg)', marginLeft: 5 }} />
          </span>
        </div>
      )}
      ...
      {showAll && (
        <div className={styles.toggleButton} onClick={toggleTags}>
          <span >
            收起
            <DoubleRightOutlined style={{ transform: 'rotate(-90deg)', marginLeft: 5 }} />
          </span>
        </div>
      )}
    </div>
